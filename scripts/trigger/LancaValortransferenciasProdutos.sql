CREATE TRIGGER LancaValortransferenciasProdutos ON transferenciasProdutos AFTER UPDATE AS 
BEGIN
IF (SELECT TRIGGER_NESTLEVEL()) > 2 return
	BEGIN
		DECLARE @idTransf AS INT, @totalVenda AS DECIMAL(16,4), @totalCompra AS DECIMAL(16,4)
		SELECT @idTransf = idTransf FROM inserted
		...
		SET totalVenda = CASE WHEN @totalVenda IS NULL THEN 0 ELSE @totalVenda END
		   ,totalCompra = CASE WHEN @totalCompra IS NULL THEN 0 ELSE @totalCompra END
